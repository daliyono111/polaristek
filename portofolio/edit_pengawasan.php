...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    echo "<script>alert('ID Proyek Pengawasan tidak ditemukan!'); window.location='bidang_pengawasan.php';</script>";
    exit();
}

$id = mysqli_real_escape_string($conn, $_GET['id']);

// Ambil data berdasarkan ID dari tabel portofolio_pengawasan
$query_data = mysqli_query($conn, "SELECT * FROM portofolio_pengawasan WHERE id = '$id'");
$data = mysqli_fetch_assoc($query_data);

if (!$data) {
    echo "<script>alert('Data tidak ditemukan di database!'); window.location='bidang_pengawasan.php';</script>";
    exit();
}
...
if (isset($_POST['update'])) {
    ...
    if (empty($pesan)) {
        $query = "UPDATE portofolio_pengawasan SET kategori = '$kategori', nama_proyek = '$nama_proyek', deskripsi = '$deskripsi' $update_bagian_foto WHERE id = '$id'";
        $result = mysqli_query($conn, $query);
        
        if ($result) {
            echo "<script>alert('Data portofolio pengawasan beserta slider foto berhasil diperbarui!'); window.location='bidang_pengawasan.php';</script>";
            exit();
        } else {
...
<head>
    ...
    <title>Edit Portofolio Bidang Pengawasan - PT. POLARISTEK ADHI PERSADA</title>
    <!-- Bootstrap 5 CSS -->
        ...
        <div class="col-md-7">
            <div class="card shadow border-0">
                <div class="card-header bg-dark text-white text-center py-3">
                    <h4 class="mb-0 fw-bold">Form Edit Portofolio Bidang Pengawasan</h4>
                </div>
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="kategori" name="kategori" required>
                                <option value="" disabled>-- Pilih Kategori Pengawasan --</option>
                        ...
                        <div class="d-grid gap-2">
                            ...
                            <a href="bidang_pengawasan.php" class="btn btn-secondary">Kembali ke Halaman Pengawasan</a>
                        </div>
                    </form>
