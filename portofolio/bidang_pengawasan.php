...
<head>
    ...
    <title>Bidang Pengawasan - PT. POLARISTEK ADHI PERSADA</title>
    <!-- Bootstrap 5 CSS -->
...
<header class="hero-section text-center">
    <div class="container">
        <h1 class="fw-bold display-5">PORTOFOLIO BIDANG PENGAWASAN</h1>
        <p class="lead text-light">Eksplorasi hasil karya pengawasan dan manajemen konstruksi profesional kami.</p>
    </div>
</header>
    ...
    <div class="container">
        ...
        <?php if (isset($_SESSION['status_login']) && $_SESSION['status_login'] == true): ?>
            <div class="mb-4 text-end">
                <a href="tambah_pengawasan.php" class="btn btn-warning fw-bold text-dark px-4 shadow-sm" style="border-radius: 6px;">
                    <i class="fas fa-plus-circle me-2"></i> Tambah Data Pengawasan
                </a>
            </div>
        <?php endif; ?>
        ...
        <div class="row g-4" id="portfolioList">
            <?php
            // Mengambil data dari tabel portofolio_pengawasan
            $query = mysqli_query($conn, "SELECT * FROM portofolio_pengawasan ORDER BY id DESC");
            if (mysqli_num_rows($query) > 0) {
                            ...
                            <div class="modal-content border-0 shadow-lg">
                                ...
                                <div class="modal-body p-4">
                                    <div class="row align-items-center g-4">
                                        ...
                                        <div class="col-md-6">
                                            <span class="badge bg-warning text-dark mb-2 px-2 py-1 fw-bold text-uppercase" style="font-size: 10px; letter-spacing: 0.5px;"><?= htmlspecialchars($row['kategori']); ?> / Pengawasan</span>
                                ...
                                <div class="modal-footer bg-light py-2 px-4 d-flex justify-content-between align-items-center">
                                    <div>
                                        <?php if (isset($_SESSION['status_login']) && $_SESSION['status_login'] == true): ?>
                                            <a href="edit_pengawasan.php?id=<?= $row['id']; ?>" class="btn btn-outline-secondary btn-sm px-3 fw-semibold text-dark border-secondary-subtle" style="font-size: 13px;"><i class="fas fa-edit me-1 text-muted"></i> Edit</a>
                                            <a href="proses_hapus_pengawasan.php?id=<?= $row['id']; ?>" class="btn btn-outline-danger btn-sm px-3 fw-semibold border-danger-subtle" style="font-size: 13px;" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')"><i class="fas fa-trash me-1"></i> Hapus</a>
                                        <?php endif; ?>
                                    </div>
                ...
                echo '<div class="col-12 text-center"><p class="text-muted fst-italic">Belum ada data portofolio pengawasan yang diinput ke dalam database.</p></div>';
            }
            ?>
        </div>
